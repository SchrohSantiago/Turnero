...
from modelos.agenda_medicos import obtener_agenda_medicos, validar_turno_existente, agregar_agenda_medico, formato_hora, modificar_horarios_agenda, desabilitar_agenda_medico
...
def obtener_agenda_medicos_json():
    ...
    if len(agenda_medicos) > 0:
        return jsonify([agenda.agenda_dict() for agenda in agenda_medicos]),200
    else:
...
def agregar_agenda_medico_json():
    ...
    if request.is_json:
        ...
        if 'id_medico' in data and 'dia_numero' in data and 'hora_inicio' in data and 'hora_fin' in data:
                    ...
                    if formato_hora(data['hora_inicio']) and formato_hora(data['hora_fin']):
                        if data['hora_inicio'] < data['hora_fin']:
                          if validar_turno_existente(data['id_medico'],data['dia_numero']):  
                                ...
                                return jsonify(agenda_medico.agenda_dict()),200
                          else:
...
def modificar_agenda_medico_json(id_search):
    ...
    if request.is_json:
        data = request.get_json()
        if 'dia_numero' in data and 'hora_inicio' in data and 'hora_fin' in data:
            if obtener_medico_id(id_search):
                if 'dia_numero' in data:
...
def desabilitar_agenda_medico_json(id_search):
    ...
    if obtener_medico_id(id_search):
        ...
        return jsonify({'exito':'eliminadas correctamente'}),200
    else:
        return jsonify({'error':'medico no encontrado'}),404